                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                            ...
                            <div className="bg-white rounded-xl shadow-sm p-6">
                                ...
                                <p className="text-2xl font-bold text-gray-900">₹{stats?.availableBalance?.toFixed(2) || '0.00'}</p>
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
                            ...
                            <div className="flex items-center justify-between text-sm text-gray-600 pt-4 border-t">
                                ...
                                <span>Revenue: ₹{monthlyData.reduce((a, b) => a + b.earnings, 0)}</span>
                            </div>
                        </div>
                                    ...
                                    {recentSales.length === 0 ? (
                                        ...
                                        recentSales.slice(0, 5).map((sale, index) => (
                                            <div key={index} className="p-4 flex items-center justify-between hover:bg-gray-50">
                                                ...
                                                <span className="text-sm font-medium text-green-600">+₹{sale.amount.toFixed(2)}</span>
                                            </div>
                                        ))
                                    )}
                                </div>
                                        ...
                                        notes.slice(0, 5).map((note) => (
                                            <div key={note.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
                                                <div className="flex items-center">
                                                    ...
                                                    <div><p className="text-sm font-medium text-gray-900">{note.title}</p><p className="text-xs text-gray-500">₹{note.price} - {note.purchaseCount || 0} sales</p></div>
                                                </div>
                                ...
                                orders.map((order) => (
                                    <div key={order.id} className="p-6 flex items-center justify-between hover:bg-gray-50">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className="text-lg font-bold text-green-600">+₹{order.amount?.toFixed(2) || '0.00'}</span>
                                        </div>
                                    </div>
                                ))
                            )}
                            ...
                            {notes.length === 0 ? (
                                ...
                                notes.map((note) => (
                                    <div key={note.id} className="p-6 flex items-center justify-between hover:bg-gray-50">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <span className="text-lg font-bold text-gray-900">₹{note.price?.toFixed(2) || '0.00'}</span>
                    ...
                    <div className="space-y-8">
                        <div className="bg-white rounded-xl shadow-sm p-6">
                                ...
                                {monthlyData.map((data, index) => (
                                    <div key={index} className="flex flex-col items-center flex-1">
                                        <div className="w-full bg-brand-500 rounded-t-md hover:bg-brand-600 relative group" style={{ height: `₹{(data.sales / maxSales) * 100}%`, margin: '0 4px' }}>
                                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap">₹{data.earnings}</div>
                                        </div>
                                        <span className="text-xs text-gray-500 mt-2">{data.month}</span>
                                    </div>
                                ))}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t">
                                ...
                                <div className="text-center"><p className="text-2xl font-bold text-gray-900">₹{monthlyData.reduce((a, b) => a + b.earnings, 0)}</p><p className="text-sm text-gray-500">Total Revenue</p></div>
                                <div className="text-center"><p className="text-2xl font-bold text-gray-900">₹{Math.round(monthlyData.reduce((a, b) => a + b.earnings, 0) / monthlyData.reduce((a, b) => a + b.sales, 0))}</p><p className="text-sm text-gray-500">Avg. Order Value</p></div>
                            ...
                            <div className="space-y-4">
                                {notes.sort((a, b) => (b.purchaseCount || 0) - (a.purchaseCount || 0)).slice(0, 5).map((note, index) => (
                                    <div key={note.id} className="flex items-center">
                                        ...
                                        <div className="text-right"><p className="font-bold text-gray-900">₹{((note.purchaseCount || 0) * (note.price || 0)).toFixed(2)}</p></div>
                                    </div>
                                ))}
